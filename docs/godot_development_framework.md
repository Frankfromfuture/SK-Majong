...
目标：做出可玩的单场武将对战。玩家每回合抓 3 打 3，通过顺子、刻子、对子+单张、散牌触发战斗效果，并用明组、暗组和将牌组成胡牌大招。
...
- 每回合抓 3 后显示 3 张候选牌，打出 3 张后手牌回到 13。
- 无锁牌逻辑。
- 打出的 3 张可被判定为刻子、顺子、对子 + 单张、散牌。
- 顺子和刻子进入 `OpenMeldLedger` 明组区。
- 当前手牌中的顺子和刻子可作为暗组候选。
...
- `1.0` 实际启用玩家 HP、防御、资金、敌方 HP、敌方攻击倒计时、手牌、抓牌、打出 3 张、明组区、暗组候选、将牌候选、向听数、大招和结束战斗；`3.0` 花季 Buff、`4.0` 武将/武器、`5.0` 消耗品、`6.0` 关卡提示先预留独立节点。
